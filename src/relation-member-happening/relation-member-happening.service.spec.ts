...
describe('Relation Member Happening Service', function() {
  ...
  beforeEach(function() {
    ...
    relationMemberHappeningService = DIContainer.get<RelationMemberHappeningService>(
      IDENTIFIER.RelationMemberHappeningService,
    );

    relationMemberHappeningRepository = DIContainer.get<RelationMemberHappeningRepository>(
  ...
  describe('Creating new happening', function() {
    it('New members should be unique for happening', function() {
      ...
      const firstParticipantListUniqueLinkData = relationMemberHappeningService.getDetailedParticipantListInformation(
        firstRelationId,
      );

      const secondParticipantListUniqueLinkData = relationMemberHappeningService.getDetailedParticipantListInformation(
        secondRelationId,
      );
  ...
  describe('Get data of member information view', function() {
    it('Data should has correct member and happening name', function() {
      ...
      const billParticipant = relationMemberHappeningService.addParticipant(
        relationId,
        MEMBER_NAME,
      );

      const memberInformationView = relationMemberHappeningService.getDataView(
        billParticipant.relationId,
      );
  ...
  describe('Getter of IParticipantUniqueLinkData', function() {
    it('Returned list should be less by 1 member by ORGANISER', function() {
      ...
      PARTICIPANT_INITIAL_LIST_MOCK.map(({ name }) =>
        relationMemberHappeningService.addParticipant(relationId, name),
      );

      const participantUniqueLinkData = relationMemberHappeningService.getDetailedParticipantListInformation(
        relationId,
      );
  ...
  describe('Get matched member', function() {
    it('Should returned matched member if happening is published', function() {
      const memberRepository: MemberRepository = DIContainer.get<MemberRepository>(
      ...
      const memberInstanceList = PARTICIPANT_INITIAL_LIST_MOCK.map(({ name }) =>
        relationMemberHappeningService.addParticipant(relationId, name),
      );
    ...
    describe('for two unique list member of happening', function() {
      ...
      before(function() {
        ...
        const firstRelationId = relationMemberHappeningService.createOwnerRelationOfHappening();
        firstMemberInstanceList = FIRST_PARTICIPANT_INITIAL_LIST_MOCK.map(({ name }) =>
          relationMemberHappeningService.addParticipant(firstRelationId, name),
        );

        const secondRelationId = relationMemberHappeningService.createOwnerRelationOfHappening();
        secondMemberInstanceList = SECOND_PARTICIPANT_INITIAL_LIST_MOCK.map(({ name }) =>
          relationMemberHappeningService.addParticipant(secondRelationId, name),
        );
      ...
      it('Matched member should be from list of happening', function() {
        const firstMembersListAbleToRandom = MatchingMemberService.filterMembersWhoAbleToRandom(
          firstHappening.getMemberList(),
        );

        assert.strictEqual(
          true,
          firstMembersListAbleToRandom.some(member =>
            firstMembersListAbleToRandom.some(el => el.id === member.MatchedMemberId),
          ),
        );
      });
      ...
      it('Another list member of happening should be has the same state', function() {
        ...
        assert.strictEqual(
          true,
          secondMemberInstanceList.some(initMember =>
            secondMemberAfterEvent.some(
              member =>
                member.id === initMember.id &&
                member.MatchedMemberId === initMember.MatchedMemberId,
            ),
          ),
        );
      });
  ...
  describe('Generate detailed participant list information', function() {
    ...
    beforeEach(function() {
      const relationId = relationMemberHappeningService.createOwnerRelationOfHappening();
      participantUniqueLinkData = relationMemberHappeningService.generateDetailedParticipantListInformation(
        relationId,
        MOCK_BODY,
      );

      participationHappening = relationMemberHappeningService.getDataView(
        participantUniqueLinkData[0].uniqueLink,
      );
    });

    it('Should generated list with the same names', function() {
      participantUniqueLinkData.map(({ name }) =>
        assert.strictEqual(true, MOCK_BODY.participantList.some(el => el.name === name)),
      );
    });
    ...
    it('getMatchedMember should return object by uniqueLink', function() {
      const memberView = relationMemberHappeningService.getMatchedMember(
        participantUniqueLinkData[0].uniqueLink,
      );
