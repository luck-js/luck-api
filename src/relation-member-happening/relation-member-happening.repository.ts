...
import RelationMemberHappeningModel, {
  IRelationMemberHappening,
} from './relation-member-happening.model';
...
export class RelationMemberHappeningRepository {
  constructor(
    private list: IRelationMemberHappening[] = [],
    private relationMemberHappeningFactory: RelationMemberHappeningFactory,
  ) {}

  public add(
    relationMemberHappening: IRelationMemberHappening,
  ): Observable<IRelationMemberHappening> {
  ...
  public getByIndex(id: string): Observable<RelationMemberHappening> {
    return from(RelationMemberHappeningModel.findOne({ id }, null, { limit: 1 }).exec()).pipe(
      map(relationMemberHappening =>
        this.relationMemberHappeningFactory.recreate(relationMemberHappening),
      ),
    );
  }
}
